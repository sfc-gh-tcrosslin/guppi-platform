-- =============================================================================
-- guppi-platform v3.8.0 — Engine Seed 02: Rules
...
USING (
  ...
  FROM VALUES
    ...
    ('RULE-029', 'platform', 'ALL', NULL, NULL,
     'TRUE', 'block', FALSE,
     'Artifact IDs are globally unique. All creation flows through CREATE_ARTIFACT (or owner-procs SUBMIT_INITIATIVE/ROCKY_EXECUTE) which allocate atomically from ID_CONVENTIONS and refuse existing IDs. Direct INSERT on ARTIFACTS is revoked from all roles except the table owner. Enforced by single write path + DUPLICATE_ID_SCREAM_V tripwire (Snowflake does not enforce PK/UNIQUE).')
